...
curl -XGET "http://localhost:9200/http_access_logs_summary/_search?pretty" -H 'Content-Type: application/json' -d'{  "size": 0,  "_source": true,  "aggs": {    "request_methods_usage": {      "terms": {        "field": "http.request.method"      },      "aggs": {        "user_agents": {          "terms": {            "field": "user_agent.original.keyword"          }        }      }    }  }}'
